...
template <bool iswa>
llm_build_lizzy<iswa>::llm_build_lizzy(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Lizzy uses post-attention norm (only if tensor exists)
        ...
        // Lizzy uses post-MLP norm (only if tensor exists)
...
template struct llm_build_lizzy<false>;
template struct llm_build_lizzy<true>;
